...
const saveCanvas = () => {
  ...
  const cutImgUrl = canvasDom.toDataURL()
...
const saveFocusText = () => {
  ...
  // 考虑文本换行的情况
  const arr = textConfig.dom.innerText ? textConfig.dom.innerText.split('\n') : []
...
const rectMousemoveFun = e => {
  ...
  if (rectConfig.canDrag) {
    ...
    const width = Math.abs(nowX - rectConfig.originX)
    const height = Math.abs(nowY - rectConfig.originY)
...
const circleMousemoveFun = e => {
  ...
  if (circleConfig.canDrag) {
    ...
    const nowX = e.clientX
    const nowY = e.clientY
    const radiusX = Math.abs((nowX - circleConfig.originX) / 2)
    const radiusY = Math.abs((nowY - circleConfig.originY) / 2)
...
const arrowMousemoveFun = e => {
  ...
  if (arrowConfig.canDrag) {
    ...
    // 起点
    const moveX = (arrowConfig.originX - getCanvasWH().x) * ratio
    const moveY = (arrowConfig.originY - getCanvasWH().y) * ratio
    // 终点
    const lineX = (e.clientX - getCanvasWH().x) * ratio
    const lineY = (e.clientY - getCanvasWH().y) * ratio
    // 斜线与X轴的角度。这里用弧度，下面的sin和cos也都用弧度。要么都用角度也可以
    const lineangle = Math.atan2(lineY - moveY, lineX - moveX)
    const angle = lineangle
    const arrowSize = arrowConfig.size * ratio
    // 让箭头更尖
    const arrowSizeHalf = arrowSize / 2
    // x1 = lineX + arrowSize * Math.sin(angle);
    // y1 = lineY - arrowSize * Math.cos(angle);
    const x1 = lineX + arrowSizeHalf * Math.sin(angle)
    const y1 = lineY - arrowSizeHalf * Math.cos(angle)

    const x2 = lineX + arrowSize * Math.cos(angle)
    const y2 = lineY + arrowSize * Math.sin(angle)

    // x3 = lineX - arrowSize * Math.sin(angle);
    // y3 = lineY + arrowSize * Math.cos(angle);
    const x3 = lineX - arrowSizeHalf * Math.sin(angle)
    const y3 = lineY + arrowSizeHalf * Math.cos(angle)
...
const textMousedownFun = e => {
  ...
  // padding+border
  const pt = parseFloat(textConfig.dom.style.paddingTop) + parseFloat(textConfig.dom.style.borderWidth)
  const pl = parseFloat(textConfig.dom.style.paddingLeft) + parseFloat(textConfig.dom.style.borderWidth)
...
undo.addEventListener('click', e => {
  ...
  if (undo.classList.contains('undo-disabled')) {
    ...
    const src = global.historyArr[global.historyArr.length - 1]
...
function copyImgToClipboard() {
  ...
  // bug的原因：多个屏幕的document为相互独立的（因为主进程中用的map，每个window都载入页面），在没有截图的屏幕双击拷贝的是当前屏幕的document的canvas，而不是截图的屏幕的document的canvas
  const url = localStorage.cutImgUrl || ''
